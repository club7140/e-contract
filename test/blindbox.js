...
contract("block", async (accounts) => {
  ...
  it("tokenURI will be blindbox url", async () => {
    let block = await Block.deployed();
    let blindTokenURI = "BlindTokenURI";
    await block.setBlindTokenURI(blindTokenURI, {from: accounts[0]});
    await block.claimNFT({value: 1e17, from: accounts[1]});
    let tokenID = await block.tokenOfOwnerByIndex.call(accounts[1], 0);
    let tokenURI = await block.tokenURI.call(tokenID);
    assert.equal(tokenURI, blindTokenURI, "blindbox url not work");
  });

  it("tokenURI will be normal url", async () => {
    let block = await Block.deployed();
    let baseURI = "BaseURI";
    await block.setBaseURI(baseURI, {from: accounts[0]});
    let tokenID = await block.tokenOfOwnerByIndex.call(accounts[1], 0);
    // open _blindBoxOpened
    await block.setBlindBoxOpened(true, {from: accounts[0]})

    let tokenURI = await block.tokenURI.call(tokenID);
    assert.equal(tokenURI, `${baseURI}?id=${tokenID}`, "blindbox url not work");
  });
})